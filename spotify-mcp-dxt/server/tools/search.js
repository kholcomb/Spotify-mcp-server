...
export class SearchTool {
    spotifyClient;
    name = 'search';
    description = 'Search Spotify catalog for tracks, albums, artists, playlists, and shows';
    ...
    async execute(input) {
        try {
            ...
            const results = await this.spotifyClient.search(params.query, params.types, searchOptions);
...
export class RecommendationsTool {
    ...
    async execute(input) {
        try {
            ...
            if (params.market)
                options.market = params.market;
            if (params.seedTracks)
                options.seed_tracks = params.seedTracks.join(',');
            if (params.seedArtists)
                options.seed_artists = params.seedArtists.join(',');
            if (params.seedGenres)
                options.seed_genres = params.seedGenres.join(',');
            // Add audio feature targets
            if (params.targetAcousticness !== undefined)
                options.target_acousticness = params.targetAcousticness;
            if (params.targetDanceability !== undefined)
                options.target_danceability = params.targetDanceability;
            if (params.targetEnergy !== undefined)
                options.target_energy = params.targetEnergy;
            if (params.targetInstrumentalness !== undefined)
                options.target_instrumentalness = params.targetInstrumentalness;
            if (params.targetLiveness !== undefined)
                options.target_liveness = params.targetLiveness;
            if (params.targetLoudness !== undefined)
                options.target_loudness = params.targetLoudness;
            if (params.targetSpeechiness !== undefined)
                options.target_speechiness = params.targetSpeechiness;
            if (params.targetTempo !== undefined)
                options.target_tempo = params.targetTempo;
            if (params.targetValence !== undefined)
                options.target_valence = params.targetValence;
            if (params.targetPopularity !== undefined)
                options.target_popularity = params.targetPopularity;
 ...
 */
export function createSearchTools(spotifyClient) {
    return [
