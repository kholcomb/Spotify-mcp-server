 ...
 */
export class GetQueueTool {
    spotifyClient;
    name = 'get_queue';
    description = 'Get the current Spotify playback queue';
    inputSchema = z.object({
 ...
 */
export class AddPlaylistToQueueTool {
    spotifyClient;
    name = 'add_playlist_to_queue';
    description = 'Add tracks from a playlist or album to the queue';
    inputSchema = z.object({
    ...
    async execute(input) {
        try {
            ...
            if (params.uri.includes('playlist')) {
                const playlistTracks = await this.spotifyClient.getPlaylistTracks(this.extractId(params.uri), { limit: params.limit, offset: params.offset });
                tracks = playlistTracks.items.map(item => item.track).filter(track => track !== null);
            }
            else if (params.uri.includes('album')) {
                const albumTracks = await this.spotifyClient.getAlbumTracks(this.extractId(params.uri), { limit: params.limit, offset: params.offset });
 ...
 */
export function createQueueTools(spotifyClient) {
    return [
