...
import { z } from 'zod';
import { HSMConfigurations, createHSMManager } from '../security/hsmManager.js';
import { RedirectUriValidator } from '../config/security.js';
/**
...
export class AuthManager {
    ...
    static SPOTIFY_AUTH_URL = 'https://accounts.spotify.com/authorize';
    static SPOTIFY_TOKEN_URL = 'https://accounts.spotify.com/api/token';
    static SCOPES = [
    ...
    constructor(config, logger) {
        ...
        this.hsmManager = createHSMManager(HSMConfigurations.autoDetect(), logger, {
    ...
    async initializeHSM() {
        try {
            ...
            const spotifyKey = existingKeys.find(key => key.attributes?.purpose === 'spotify-token-encryption');
            if (spotifyKey) {
     ...
     */
    async exchangeCodeForTokens(userId, code, state) {
        try {
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new AuthError(`Spotify authentication failed: ${data?.error_description || error.message}`, details);
            }
     ...
     */
    async refreshAccessToken(userId, currentTokens) {
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new AuthError(`Token refresh failed: ${data?.error_description || error.message}`, details);
            }
    ...
    async revokeTokens(userId) {
        try {
            ...
            if (tokens?.accessToken) {
                ...
                try {
                    await axios.post('https://accounts.spotify.com/api/token/revoke', `token=${tokens.accessToken}`, {
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
     ...
     */
    deriveEncryptionKey(secret, salt) {
    ...
    async storePKCEData(userId, data) {
