...
export class RedirectUriValidator {
  ...
  static generateSecureConfig(
    ...
    const redirectUri: RedirectUriConfig = {
      uri: baseUri,
  ...
  private validateProtocol(parsedUri: URL, results: ValidationResult): void {
  ...
  private validateDomain(parsedUri: URL, results: ValidationResult): void {
    ...
    if (this.config.redirectUri.allowedDomains.length > 0) {
      const isAllowed = this.config.redirectUri.allowedDomains.some(domain => 
  ...
  private validateSecurityPractices(parsedUri: URL, results: ValidationResult): void {
  ...
  private validateProductionRequirements(parsedUri: URL, results: ValidationResult): void {
  ...
  private sanitizeUriForLogging(uri: string): string {
    try {
