...
export class CallbackServer {
  ...
  private readonly maxRequestSize = 8192; // 8KB

  constructor(authManager: AuthManager, logger: Logger, port: number = 8080) {
   ...
   */
  private validateRequest(req: IncomingMessage, res: ServerResponse): boolean {
   ...
   */
  private async handleCallback(url: URL, res: ServerResponse): Promise<void> {
    ...
    if (error) {
      ...
      const html = this.generateErrorPage(
        'Authorization Failed',
        `Spotify authorization failed: ${errorDescription || error}`,
    ...
    try {
      ...
      if (result.success) {
        ...
        const html = this.generateSuccessPage(
          'Authorization Successful!',
          'Your Spotify account has been successfully connected.',
          'You can now close this window and return to your AI assistant.'
        );
      ...
      const html = this.generateErrorPage(
        ...
        'Failed to complete authorization with Spotify.',
        'Please close this window and try again.'
      );
   ...
   * Handle 404 errors
   */
  private async handle404(res: ServerResponse): Promise<void> {
    const html = this.generateErrorPage(
      'Not Found',
      'The requested page was not found.',
      'This server only handles OAuth callbacks.'
    );
   ...
   */
  private async handleError(res: ServerResponse, _error: unknown): Promise<void> {
    const html = this.generateErrorPage(
      'Server Error',
   ...
   */
  private generateSuccessPage(title: string, message: string, instruction: string): string {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
   ...
   */
  private generateErrorPage(title: string, message: string, instruction: string): string {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
